...
if __name__ == '__main__':
    ...
    gerador_treino = data_generator_train.flow_from_directory(dir_treino, batch_size=32, target_size=(224,224), class_mode='sparse')
    gerador_teste = data_generator_test.flow_from_directory(dir_teste, batch_size=32, target_size=(224,224), class_mode='sparse')

    plt.imshow(gerador_treino[0][0][0])
    plt.show()

    #Obtendo um modelo de rede neural (Xception)
    model = keras.applications.xception.Xception(input_shape=(224,224,3), include_top=False, weights='imagenet')
